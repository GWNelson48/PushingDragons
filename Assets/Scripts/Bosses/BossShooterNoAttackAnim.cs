...
public class BossShooterNoAttackAnim : MonoBehaviour
{
    [Header("Projectile 1")]
    ...
    void Shoot()
    {
        ...
        Invoke("Shoot3", timeBetweenVolleys + timeBetweenVolleys);
        Invoke("Shoot4", timeBetweenVolleys + timeBetweenVolleys + timeBetweenVolleys);
    ...
    void Shoot4()
    {
        foreach (GameObject gun in gunArray4)
        {
            GameObject shot = Instantiate(projectilePrefab, gun.transform.position, Quaternion.identity);
            Vector2 shotDirection = (gun.transform.localRotation * Vector2.down).normalized;
            Rigidbody2D rb = shot.GetComponent<Rigidbody2D>();
